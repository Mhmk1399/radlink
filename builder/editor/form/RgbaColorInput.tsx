...
export function RgbaColorInput({
  ...
  return (
    <div ref={wrapperRef} className={["relative flex items-center gap-3", className].join(" ")}>
      <button
        type="button"
        onClick={() => {
      ...
      <input
        ...
        className={[
          inputClassName,
          isValidDraft ? "" : "border-red-300 focus:border-red-400 focus:ring-red-100",
        ].join(" ")}
        dir="ltr"
      ...
      {open && (
        <div
          className={[
            "absolute top-full z-[520] mt-2 rounded-2xl border border-neutral-200 bg-white p-4 shadow-[0_24px_80px_-18px_rgba(0,0,0,0.25)]",
            panelClassName,
          ].join(" ")}
        >
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-2">
              <HiOutlineEyeDropper size={14} className="shrink-0 text-neutral-500" />
          ...
          <div className="flex items-stretch gap-3">
            ...
            >
              <Swatch color={formatCssColor(parsedColor)} className="h-16 w-16 rounded-2xl" />
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="rounded-xl border border-neutral-100 bg-neutral-50 p-2.5">
                <div className="mb-2 flex items-center justify-between text-[11px] font-bold text-neutral-500">
                  <span>Opacity</span>
                  <span className="font-mono">{alphaLabel(parsedColor.a)}</span>
                </div>
            ...
            <div className="mt-4">
              ...
              <div className="grid grid-cols-5 gap-2">
                {recentColors.map((recent, index) => {
                  const active =
                    recent.toLowerCase() === draftValue.trim().toLowerCase();
                  return (
                    <button
                      key={`${recent}-${index}`}
                      type="button"
