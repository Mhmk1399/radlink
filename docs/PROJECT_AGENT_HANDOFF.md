...
- Product-card items are synchronized into `Product` documents whenever a page is created or updated. Builder products use a unique `(page, sourceBlockInstanceId, sourceItemId)` identity, store owner/page/source metadata and one image URL, link the matching uploaded `File` through `imageFile`, and remove stale products when cards disappear. Product list APIs populate owner/page/file and scope non-admin users to their own products.
- `ProductsSection` uses a custom single-image form field for authenticated upload, removal, and replacement. Product thumbnails in table and row-view renderers open the shared `ImagePreviewModal`; searchable owner/page filters send relational IDs to the backend and operate across the complete paginated dataset.
