...
### یادداشت تغییرات مهم 2026-09-10
...
- هر کاربری که مجوز `admin.users:create` یا `admin.users:update` دارد باید در فرم ساخت/ادیت کاربر به فیلدهای کامل همان عملیات دسترسی داشته باشد؛ این موضوع دیگر وابسته به نقش نماینده/مدیر نماینده نیست.
- نقش `superAdmin` در UI با عنوان `R A D` نمایش داده می‌شود و ساختن یا تبدیل کردن کاربر به این نقش فقط برای خود `superAdmin` مجاز است.
- اگر نماینده یا مدیر نماینده کاربر جدید بسازد، نماینده همان سازنده باید به‌صورت پیش‌فرض روی کاربر جدید قرار بگیرد و backend هم در نبود `agentid` همین fallback را اعمال کند.
- `DynamicTable.renderFormField` علاوه بر `formData`، رکورد اصلی/`selectedRow` را هم دریافت می‌کند تا فیلدهای سفارشی داخل مودال ادیت بتوانند به `_id` رکورد دسترسی داشته باشند.
...
قرارداد ساخت و ویرایش کاربر:

- نمایش فیلدهای کامل فرم کاربر در `UsersSection` باید بر اساس permission انجام شود: `admin.users:create` برای فرم ساخت و `admin.users:update` برای فرم ادیت.
- کاربری که این مجوزها را دارد باید بتواند فیلدهای رمز عبور، نقش، وضعیت، نماینده، محدودیت‌ها و سایر فیلدهای مدیریتی را در همان عملیات ببیند و ارسال کند، حتی اگر نقش خودش `agent` یا `agentManager` باشد.
- تنها استثنای نقش‌ها `superAdmin`/`R A D` است: گزینه ساخت یا تغییر نقش به `superAdmin` فقط باید برای خود `superAdmin` نمایش داده و در API پذیرفته شود.
- اگر سازنده کاربر نقش `agent` یا `agentManager` داشته باشد، فرم ساخت باید `agentid` همان نماینده سازنده را پیش‌فرض کند و `POST /api/users` هم اگر `agentid` خالی بود همان Agent فعال سازنده را ثبت کند.
- اگر سازنده نقش نماینده‌ای داشته باشد ولی Agent فعال برای او پیدا نشود، ساخت کاربر باید با خطای کنترل‌شده متوقف شود و کاربر جدید بدون نماینده ساخته نشود.
...
در حالت `mode=user-form-options` روی `GET /api/agents`، اگر کاربر مجوز ساخت یا ویرایش کاربر داشته باشد، endpoint می‌تواند گزینه‌های نماینده لازم برای فرم کاربران را برگرداند. برای `agent` و `agentManager` در این حالت، خود نماینده سازنده هم باید داخل خروجی باشد تا فیلد نماینده کاربر جدید به‌صورت پیش‌فرض و قابل مشاهده پر شود.
...
- فیلدهای سفارشی فرم (`renderFormField`) فقط نباید به `formData` متکی باشند؛ `DynamicTable` باید `selectedRow` را هم پاس بدهد تا کنترل‌هایی مثل نمایش صفحات کاربر در مودال ادیت به `_id` رکورد اصلی دسترسی داشته باشند.
...
- UsersSection برای هر ردیف و داخل مودال مشاهده/ادیت، نمایش lazy صفحات کاربر دارد. این لیست فقط با کلیک کاربر از API صفحات گرفته می‌شود تا جدول کاربران سنگین نشود.
